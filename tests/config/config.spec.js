...
const { getByCityNameUrl, getByZipCodeUrl } = require('../../src/data-access/config');
...
test.each([
    ...
    const url = getByCityNameUrl(cityName, { units });
...
test.each([
    ...
    const url = getByZipCodeUrl(zipCode, countryCode, { units });
    expect(url).toEqual(expected);
});
